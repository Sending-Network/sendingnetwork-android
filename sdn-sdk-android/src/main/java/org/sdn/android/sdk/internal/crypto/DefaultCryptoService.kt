...
import org.sdn.android.sdk.api.session.room.model.RoomHistoryVisibilityContent
import org.sdn.android.sdk.api.session.room.model.RoomMemberContent
import org.sdn.android.sdk.api.session.room.model.shouldShareHistory
import org.sdn.android.sdk.api.session.sync.model.SyncResponse
...
import org.sdn.android.sdk.internal.crypto.model.MXKey.Companion.KEY_SIGNED_CURVE_25519_TYPE
import org.sdn.android.sdk.internal.crypto.model.SessionInfo
import org.sdn.android.sdk.internal.crypto.model.toRest
...
import org.sdn.android.sdk.internal.crypto.verification.DefaultVerificationService
import org.sdn.android.sdk.internal.crypto.verification.VerificationMessageProcessor
...
import org.sdn.android.sdk.internal.task.launchToCallback
...
//    val gossipingBuffer = mutableListOf<Event>()
    ...
    override fun setDeviceName(deviceId: String, deviceName: String, callback: SDNCallback<Unit>) {
        setDeviceNameTask
                .configureWith(SetDeviceNameTask.Params(deviceId, deviceName)) {
                    ...
                    this.callback = object : SDNCallback<Unit> {
                        override fun onSuccess(data: Unit) {
                            ...
                            downloadKeys(listOf(userId), true, NoOpSDNCallback())
    ...
    override fun deleteDevice(deviceId: String, userInteractiveAuthInterceptor: UserInteractiveAuthInterceptor, callback: SDNCallback<Unit>) {
        deleteDevices(listOf(deviceId), userInteractiveAuthInterceptor, callback)
    }

    override fun deleteDevices(deviceIds: List<String>, userInteractiveAuthInterceptor: UserInteractiveAuthInterceptor, callback: SDNCallback<Unit>) {
        deleteDeviceTask
    ...
    override fun fetchDevicesList(callback: SDNCallback<DevicesListResponse>) {
        getDevicesTask
                .configureWith {
                    //                    this.executionThread = TaskThread.CRYPTO
                    this.callback = object : SDNCallback<DevicesListResponse> {
                        override fun onFailure(failure: Throwable) {
                            callback.onFailure(failure)
                        }
    ...
    fun start() {
        ...
        // Just update
        fetchDevicesList(NoOpSDNCallback())
    ...
    fun ensureDevice() {
        cryptoCoroutineScope.launchToCallback(coroutineDispatchers.crypto, NoOpSDNCallback()) {
            // Open the store
            cryptoStore.open()
    ...
    override fun getCryptoDeviceInfo(deviceId: String, callback: SDNCallback<DeviceInfo>) {
        getDeviceInfoTask
     ...
     */
    override fun setDevicesKnown(devices: List<MXDeviceInfo>, callback: SDNCallback<Unit>?) {
        // build a devices map
     ...
     */
    override fun decryptEventAsync(event: Event, timeline: String, callback: SDNCallback<MXEventDecryptionResult>) {
     ...
     * @param callback the asynchronous callback.
     */
    fun checkUnknownDevices(userIds: List<String>, callback: SDNCallback<Unit>) {
    ...
    private fun pullRoomKey(requestBody: RoomKeyRequestBody) {
        ...
        val callback = object : SDNCallback<ToDeviceSyncResponse> {
            override fun onFailure(failure: Throwable) {
    ...
    override fun downloadKeys(userIds: List<String>, forceDownload: Boolean, callback: SDNCallback<MXUsersDevicesMap<CryptoDeviceInfo>>) {
        cryptoCoroutineScope.launch(coroutineDispatchers.crypto) {
            runCatching {
 ...
 * ========================================================================================== */
    ...
    override fun prepareToEncrypt(roomId: String, callback: SDNCallback<Unit>) {
        cryptoCoroutineScope.launch(coroutineDispatchers.crypto) {
