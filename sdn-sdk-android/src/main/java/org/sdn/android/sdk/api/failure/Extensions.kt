...
package org.sdn.android.sdk.api.failure

import org.sdn.android.sdk.api.auth.registration.RegistrationFlowResponse
import org.sdn.android.sdk.api.extensions.tryOrNull
import org.sdn.android.sdk.api.session.contentscanner.ContentScannerError
import org.sdn.android.sdk.api.session.contentscanner.ScanFailure
...
fun Throwable.is401() = this is Failure.ServerError &&
        ...
        error.code == SDNError.M_UNAUTHORIZED

fun Throwable.is404() = this is Failure.ServerError &&
        httpCode == HttpsURLConnection.HTTP_NOT_FOUND && /* 404 */
        error.code == SDNError.M_NOT_FOUND

fun Throwable.isTokenError() = this is Failure.ServerError &&
        (error.code == SDNError.M_UNKNOWN_TOKEN ||
                error.code == SDNError.M_MISSING_TOKEN ||
                error.code == SDNError.ORG_MATRIX_EXPIRED_ACCOUNT)

fun Throwable.isLimitExceededError() = this is Failure.ServerError &&
        httpCode == 429 &&
        error.code == SDNError.M_LIMIT_EXCEEDED
...
fun Throwable.getRetryDelay(defaultValue: Long): Long {
    return (this as? Failure.ServerError)
            ?.error
            ?.takeIf { it.code == SDNError.M_LIMIT_EXCEEDED }
            ?.retryAfterMillis
            ?.plus(100L)
            ?: defaultValue
}
...
fun Throwable.isUsernameInUse() = this is Failure.ServerError &&
        error.code == SDNError.M_USER_IN_USE

fun Throwable.isInvalidUsername() = this is Failure.ServerError &&
        (error.code == SDNError.M_INVALID_USERNAME || usernameContainsNonAsciiCharacters())

private fun Failure.ServerError.usernameContainsNonAsciiCharacters() = error.code == SDNError.M_UNKNOWN &&
        error.message == "Query parameter \'username\' must be ascii"

fun Throwable.isInvalidPassword() = this is Failure.ServerError &&
        error.code == SDNError.M_FORBIDDEN &&
...
fun Throwable.isWeakPassword() = this is Failure.ServerError &&
        error.code == SDNError.M_WEAK_PASSWORD
...
fun Throwable.isRegistrationAvailabilityError() = this is Failure.ServerError &&
        ...
        (error.code == SDNError.M_USER_IN_USE ||
                error.code == SDNError.M_INVALID_USERNAME ||
                error.code == SDNError.M_EXCLUSIVE)
