 ...
 */
package org.sdn.android.sdk.internal.session.room.send.pills

import android.text.SpannableString
import org.sdn.android.sdk.api.session.permalinks.PermalinkService
import org.sdn.android.sdk.api.session.room.send.SDNItemSpan
...
internal class TextPillsUtils @Inject constructor(
    ...
    private fun transformPills(text: CharSequence, template: String): String? {
        ...
        val pills = spannableString
                ?.getSpans(0, text.length, SDNItemSpan::class.java)
                ...
                ?.filterNot { it.span.SDNItem is SDNItem.EveryoneInRoomItem }
                ?.toMutableList()
        ...
        return buildString {
            ...
            pills.forEachIndexed { _, (urlSpan, start, end) ->
                ...
                // append the pill
                append(String.format(template, urlSpan.SDNItem.id, displayNameResolver.getBestName(urlSpan.SDNItem)))
                currIndex = end
            }
