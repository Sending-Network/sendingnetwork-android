...
internal class HomeServerAvailabilityChecker(val sessionParams: SessionParams) {
    ...
    fun check(): Boolean {
        val host = sessionParams.edgeNodeConnectionConfig.homeServerUriBase.host ?: return false
        val port = sessionParams.edgeNodeConnectionConfig.homeServerUriBase.port.takeIf { it != -1 } ?: 80
        val timeout = 30_000
        try {
            Socket().use { socket ->
