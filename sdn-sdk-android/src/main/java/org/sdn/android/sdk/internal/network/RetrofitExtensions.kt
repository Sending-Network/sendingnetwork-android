...
/**
 * Convert a retrofit Response to a Failure, and eventually parse errorBody to convert it to a [SDNError].
 */
...
/**
 * Convert a HttpException to a Failure, and eventually parse errorBody to convert it to a [SDNError].
 */
...
/**
 * Convert a okhttp3 Response to a Failure, and eventually parse errorBody to convert it to a [SDNError].
 */
...
private fun toFailure(errorBody: ResponseBody?, httpCode: Int, globalErrorReceiver: GlobalErrorReceiver?): Failure {
    ...
    try {
        val matrixError = sdnErrorAdapter.fromJson(errorBodyStr)

        if (matrixError != null) {
            // Also send following errors to the globalErrorReceiver, for a global management
            when {
                matrixError.code == SDNError.M_CONSENT_NOT_GIVEN && !matrixError.consentUri.isNullOrBlank() -> {
