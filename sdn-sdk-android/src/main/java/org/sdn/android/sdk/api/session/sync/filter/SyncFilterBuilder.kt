...
package org.sdn.android.sdk.api.session.sync.filter

import org.sdn.android.sdk.api.session.node.NodeCapabilities
import org.sdn.android.sdk.internal.session.filter.Filter
import org.sdn.android.sdk.internal.session.filter.RoomEventFilter
import org.sdn.android.sdk.internal.session.filter.RoomFilter
...
class SyncFilterBuilder {
    ...
    internal fun build(nodeCapabilities: NodeCapabilities): Filter {
        return Filter(
                room = buildRoomFilter(nodeCapabilities)
        )
    }

    private fun buildRoomFilter(nodeCapabilities: NodeCapabilities): RoomFilter {
        return RoomFilter(
                timeline = buildTimelineFilter(nodeCapabilities),
                state = buildStateFilter()
        )
    }

    private fun buildTimelineFilter(nodeCapabilities: NodeCapabilities): RoomEventFilter? {
        val resolvedUseThreadNotifications = if (nodeCapabilities.canUseThreadReadReceiptsAndNotifications) {
