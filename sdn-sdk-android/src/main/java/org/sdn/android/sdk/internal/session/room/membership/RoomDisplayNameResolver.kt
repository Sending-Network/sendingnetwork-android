...
package org.sdn.android.sdk.internal.session.room.membership
...
import org.sdn.android.sdk.internal.session.displayname.DisplayNameResolver
...
internal class RoomDisplayNameResolver @Inject constructor(
    ...
    private val roomDisplayNameFallbackProvider = sdnConfiguration.roomDisplayNameFallbackProvider
            ...
            name = inviterId
                    ?.let {
                        activeMembers.where()
                                ...
                                .findFirst()
                                ?.toSDNItem()
                                ?.let { sdnItem -> displayNameResolver.getBestName(sdnItem) }
                    }
            ...
            name = when (otherMembersCount) {
                0 -> {
                    ...
                    val leftMembersNames = roomMembers.queryLeftRoomMembersEvent()
                            .findAll()
                            .map { displayNameResolver.getBestName(it.toSDNItem()) }
    ...
    private fun resolveRoomMemberName(
        ...
        return if (isUnique) {
            displayNameResolver.getBestName(roomMemberSummary.toSDNItem())
        } else {
