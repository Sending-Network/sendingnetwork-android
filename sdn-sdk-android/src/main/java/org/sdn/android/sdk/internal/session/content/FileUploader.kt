...
import org.sdn.android.sdk.internal.network.awaitResponse
...
internal class FileUploader @Inject constructor(
    ...
    suspend fun uploadFile(
        ...
        // Check size limit
        val maxUploadFileSize = nodeCapabilitiesService.getNodeCapabilities().maxUploadFileSize

        if (maxUploadFileSize != NodeCapabilities.MAX_UPLOAD_FILE_SIZE_UNKNOWN &&
                file.length() > maxUploadFileSize) {
            // Known limitation and file too big for the server, save the pain to upload it
            throw Failure.ServerError(
                    error = SDNError(
                            code = SDNError.M_TOO_LARGE,
