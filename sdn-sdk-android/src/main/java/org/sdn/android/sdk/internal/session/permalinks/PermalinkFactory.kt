...
package org.sdn.android.sdk.internal.session.permalinks
...
import org.sdn.android.sdk.api.session.permalinks.PermalinkData
import org.sdn.android.sdk.api.session.permalinks.PermalinkParser
import org.sdn.android.sdk.api.session.permalinks.PermalinkService
import org.sdn.android.sdk.api.session.permalinks.PermalinkService.Companion.MATRIX_TO_URL_BASE
import org.sdn.android.sdk.api.session.permalinks.PermalinkService.SpanTemplateType.HTML
import org.sdn.android.sdk.api.session.permalinks.PermalinkService.SpanTemplateType.MARKDOWN
...
internal class PermalinkFactory @Inject constructor(
    ...
    fun createPermalink(id: String, forceMatrixTo: Boolean): String? {
        return when {
            ...
            !useClientFormat(forceMatrixTo) -> MATRIX_TO_URL_BASE + escape(id)
                ...
                buildString {
                    append(sdnConfiguration.clientPermalinkBaseUrl)
                    when {
    ...
    fun getLinkedId(url: String): String? {
        val clientBaseUrl = sdnConfiguration.clientPermalinkBaseUrl
        return when {
    ...
    /**
     * Get the permalink base URL according to the potential one in [SDNConfiguration.clientPermalinkBaseUrl]
    ...
    private fun baseUrl(forceMatrixTo: Boolean): String {
        return sdnConfiguration.clientPermalinkBaseUrl
                ?.takeUnless { forceMatrixTo }
                ?: MATRIX_TO_URL_BASE
    }

    private fun useClientFormat(forceMatrixTo: Boolean): Boolean {
        return !forceMatrixTo && sdnConfiguration.clientPermalinkBaseUrl != null
    }
